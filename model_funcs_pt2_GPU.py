...
def comp(out,target):
    idx = np.argmax(smax(out.cpu().data.numpy()))
...
#             return True
        ...
        #                 self.memory = Variable(torch.from_numpy(temp_mem.copy())).float()
                ...
                for i in range(self.num_hops):
                    ...
                    if LS == 0:
                        P = self.softmax(aux.view(1,-1)).view(-1,1)
                    else:
                        P = aux
...
#             target = Variable(torch.from_numpy(np.array([test_dt_bow[i,-1]])).type(torch.LongTensor))
            target = Variable(torch.from_numpy(np.array([test_dt_bow[i,-1]])).type(torch.LongTensor).cuda())
            n_corr += comp(out,target)
            if i < lim:
                print('QQ: '," ".join([idx2word[statement] for statement in test_dt_pe[i][0,:-1]]))
                print('target: ', idx2word[target.data[0]])
                print('out: ', idx2word[np.argmax(out.cpu().data.numpy())])
                if get_probs:
                    probs_list = [(out.cpu().data.numpy()[0,j],idx2word[j]) for j in range(out.cpu().data.numpy().shape[1])]
